...
describe('github/client', () => {
  ...
  describe('readFile', () => {
    it('retorna conteúdo decodificado de Base64 quando arquivo existe', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        makeOkResponse({ content: encoded + '\n', encoding: 'base64' }),
      );
    ...
    it('retorna conteúdo de arquivo em branch específico', async () => {
      ...
      mockInstallationToken();
      mockFetch.mockResolvedValueOnce(
        makeOkResponse({ content: encoded, encoding: 'base64' }),
      );
  ...
  describe('createBranch', () => {
    ...
    it('lança erro quando criação de branch falha', async () => {
      mockInstallationToken();
      mockFetch.mockResolvedValueOnce(makeOkResponse({ default_branch: 'main' }));
      mockFetch.mockResolvedValueOnce(makeOkResponse({ object: { sha: 'abc123' } }));
      ...
      const { createBranch } = await import('./client');
      await expect(createBranch('existing-branch')).rejects.toThrow('422');
    });
  });
  ...
  describe('waitForWorkflowCompletion', () => {
    ...
    it('retorna imediatamente quando run já está completo', async () => {
      const completedRun = {
        workflow_runs: [
          {
            id: 42,
  ...
  describe('commitFiles', () => {
    it('cria commit com múltiplos arquivos com sucesso', async () => {
      ...
      // Get branch ref SHA
      mockFetch.mockResolvedValueOnce(
        makeOkResponse({ object: { sha: 'head-sha-abc' } }),
      );
      // Get commit tree SHA
      mockFetch.mockResolvedValueOnce(
        makeOkResponse({ commit: { tree: { sha: 'tree-sha-xyz' } } }),
      );
    ...
    it('lança erro quando API de criação de blob falha', async () => {
      ...
      mockFetch.mockResolvedValueOnce(makeOkResponse({ commit: { tree: { sha: 'tree-sha' } } }));
      mockFetch.mockResolvedValueOnce(makeErrorResponse(422, 'Blob creation failed'));

      const { commitFiles } = await import('./client');
      await expect(
        commitFiles('my-branch', 'feat: test', [{ path: 'file.ts', content: 'code' }]),
      ).rejects.toThrow('422');
    });
  });
  ...
  describe('createPullRequest', () => {
    it('cria pull request com sucesso', async () => {
      ...
      const result = await createPullRequest('feature-branch', 'My PR', 'PR body');
    ...
    it('lança erro quando PR já existe (422)', async () => {
      ...
      const { createPullRequest } = await import('./client');
      await expect(
        createPullRequest('feature-branch', 'Title', 'Body'),
      ).rejects.toThrow('422');
    });
  });
  ...
  describe('commitFile', () => {
    it('cria commit de arquivo único com sucesso', async () => {
      ...
      mockFetch.mockResolvedValueOnce(makeOkResponse({ commit: { tree: { sha: 'tree-sha' } } }));
      // Create blob
      mockFetch.mockResolvedValueOnce(makeOkResponse({ sha: 'blob-sha' }));
      // Create tree
      mockFetch.mockResolvedValueOnce(makeOkResponse({ sha: 'new-tree-sha' }));
      // Create commit
      mockFetch.mockResolvedValueOnce(
        makeOkResponse({ sha: 'commit-sha', html_url: 'https://example.com' }),
      );
      // Update ref
      mockFetch.mockResolvedValueOnce(makeOkResponse({ ref: 'refs/heads/branch' }));

      const { commitFile } = await import('./client');
      const result = await commitFile('my-branch', 'src/file.ts', 'content here', 'feat: add');
